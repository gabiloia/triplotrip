...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    async function deletePropiedad() {
      try {
        const token = localStorage.getItem("auth_token");
        if (!token || !editingId.value) return;
        await $fetch(`https://admin.triplotrip.com/api/propiedades/${editingId.value}`, {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` }
        });
        notify("success", "Propiedad eliminada", "La propiedad fue borrada correctamente");
        ...
        console.error(err);
        notify("error", "Error al eliminar", err.message || "No se pudo eliminar");
      }
    }
    async function submitForm() {
      try {
        ...
        const url = isEditing.value ? `https://admin.triplotrip.com/api/propiedades/${editingId.value}` : `https://admin.triplotrip.com/api/propiedades`;
        ...
        notify("success", "Propiedad guardada", "Los cambios fueron guardados correctamente");
        showModal.value = false;
        form.value = { Titulo: "", Descripcion: "", FotoPrincipal: null };
        editingId.value = null;
        isEditing.value = false;
        ...
        console.error(err);
        notify("error", "Error al guardar", err.message || "No se pudo guardar");
      }
    }
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "activity" }, _attrs))} data-v-ffbc0d2d><div class="mb-4 flex justify-end" data-v-ffbc0d2d>`);
      ...
      _push(`</div><div class="masonry" data-v-ffbc0d2d><!--[-->`);
      ssrRenderList(propiedades.value, (prop, index) => {
        _push(`<div class="card-wrap" data-v-ffbc0d2d>`);
        _push(ssrRenderComponent(unref(NCard), {
          ...
          cover: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<img class="property-image"${ssrRenderAttr("alt", prop.Titulo || "Imagen")}${ssrRenderAttr("src", getImageUrl(prop.FotoPrincipal))} data-v-ffbc0d2d${_scopeId}>`);
            } else {
              return [
                createVNode("img", {
          ...
          header: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="cursor-pointer text-blue-500 underline" data-v-ffbc0d2d${_scopeId}>${ssrInterpolate(prop.Titulo)}</span>`);
            } else {
              return [
                createVNode("span", {
                  class: "cursor-pointer text-blue-500 underline",
                  onClick: ($event) => openEditModal(prop)
                }, toDisplayString(prop.Titulo), 9, ["onClick"])
              ];
            }
          }),
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<p data-v-ffbc0d2d${_scopeId}>${ssrInterpolate(prop.Descripcion)}</p>`);
            } else {
              return [
...
const ProfileActivity = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-ffbc0d2d"]]);
...
//# sourceMappingURL=Profile-lJ4qIrwf.mjs.map
